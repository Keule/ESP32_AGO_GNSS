...
struct NavigationState {
    ...
    // Protocol sends: Kp, highPWM, lowPWM, minPWM, countsPerDeg,
    //                 wasOffset (2 bytes), ackerman
    float   settings_kp;      // proportional gain from AgIO (value / 10.0)
    float   settings_ki;      // integral gain (not sent by AgIO, locally configured)
    float   settings_kd;      // derivative gain (not sent by AgIO, locally configured)
    uint16_t settings_min_pwm;// actuator min PWM from AgIO
    uint16_t settings_max_pwm;// actuator max PWM from AgIO
    uint16_t settings_counts; // sensor counts per degree from AgIO
