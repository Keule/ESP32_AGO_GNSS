...
//
// Performance optimisation: Src field is checked before the expensive
// CRC calculation. Frames from sources other than AgIO (0x7F) are
// rejected early since netProcessFrame() would discard them anyway.
// ===================================================================
bool aogValidateFrame(const uint8_t* frame, size_t frame_len,
    ...
    // Early rejection: only accept frames from AgIO (0x7F).
    // This avoids CRC calculation on broadcast echo from our own
    // module (0x7E) and other modules (0x7B, 0x79, etc.) that may
    // have hardcoded/stale CRC values.
    if (src != AOG_SRC_AGIO) return false;
...
bool tryDecodeAogHelloFromAgio(const uint8_t* payload, size_t payload_len,
    ...
    if (out) *out = msg;
    hal_log("AOG: Hello from AgIO (module=0x%02X, sp1=%u sp2=%u)",
            (unsigned)msg.moduleId, (unsigned)msg.spare1, (unsigned)msg.spare2);
    return true;
}
...
bool tryDecodeAogSteerSettingsIn(const uint8_t* payload, size_t payload_len,
    ...
    uint16_t wasOffset = (uint16_t)msg.wasOffsetLo | ((uint16_t)msg.wasOffsetHi << 8);
    hal_log("AOG: SteerSettings Kp=%u hiPWM=%u loPWM=%u minPWM=%u counts=%u wasOff=%d ackerman=%u",
            (unsigned)msg.kp, (unsigned)msg.highPWM, (unsigned)msg.lowPWM,
            (unsigned)msg.minPWM, (unsigned)msg.countsPerDeg,
            (int)wasOffset, (unsigned)msg.ackerman);
    return true;
}
