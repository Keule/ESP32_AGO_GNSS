...
// UDP Ports
// ===================================================================
constexpr uint16_t AOG_PORT_STEER = 5126;   // Module's own source port (for identification)
constexpr uint16_t AOG_PORT_GPS   = 5124;
constexpr uint16_t AOG_PORT_AGIO  = 9999;   // AgIO listens here for incoming data (send TO)
constexpr uint16_t AOG_PORT_LISTEN = 8888;  // Port module listens ON (AgIO sends here)
...
struct __attribute__((packed)) AogHelloFromAgio {
    ...
    uint8_t  spare1;         // reserved (0)
    uint8_t  spare2;         // reserved (0)
};
...
// Total payload: 8 bytes
//
// Reference: AgOpenGPS Boards/PGN.md, Autosteer_UDP_v5.ino,
//            AgOpenGPS docs/pgn-protocol.md
// ===================================================================

struct __attribute__((packed)) AogSteerSettingsIn {
    uint8_t  kp;              // proportional gain (actual = value / 10.0)
    uint8_t  highPWM;         // maximum actuator PWM (0-255)
    uint8_t  lowPWM;          // deadband / low-speed PWM (0-255)
    uint8_t  minPWM;          // absolute minimum instant-on PWM (0-255)
    uint8_t  countsPerDeg;    // steer angle sensor counts per degree
    uint8_t  wasOffsetLo;     // sensor zero offset low byte
    uint8_t  wasOffsetHi;     // sensor zero offset high byte
    uint8_t  ackerman;        // Ackerman correction (actual = value * 0.01)
};
