...
void controlUpdateSettings(uint8_t kp, uint8_t highPWM, uint8_t lowPWM,
                           uint8_t minPWM) {
    // AgIO sends Kp scaled by 10 (e.g., Kp=30 means 3.0)
    float new_kp = kp / 10.0f;

    // Map uint8 PWM values to float range 0-255
    float new_max = static_cast<float>(highPWM);
    float new_min_inst = static_cast<float>(minPWM);

    // Only update if values actually changed
    if (new_kp != s_steer_pid.kp ||
        new_max != s_steer_pid.output_max ||
        new_min_inst != s_steer_pid.output_min) {

        s_steer_pid.kp = new_kp;
        // Ki and Kd are NOT sent by AgIO — keep existing values
        s_steer_pid.output_min = new_min_inst;
        s_steer_pid.output_max = new_max;
        ...
        hal_log("Control: settings updated Kp=%.1f Ki=%.1f Kd=%.3f minPWM=%.0f maxPWM=%.0f",
                new_kp, s_steer_pid.ki, s_steer_pid.kd,
                new_min_inst, new_max);
    }
}
