...
void netProcessFrame(uint8_t src, uint8_t pgn,
    ...
    // Note: Src=0x7F filter is already enforced in aogValidateFrame(),
    // so all frames reaching here are guaranteed to be from AgIO.

    switch (pgn) {
        case PGN_HELLO_FROM_AGIO: {
            AogHelloFromAgio msg;
            if (tryDecodeAogHelloFromAgio(payload, payload_len, &msg)) {
                hal_log("NET: Hello from AgIO (module=0x%02X) -> sending ALL module hellos",
                        (unsigned)msg.moduleId);
        ...
        case PGN_STEER_SETTINGS_IN: {
            ...
            if (tryDecodeAogSteerSettingsIn(payload, payload_len, &settings)) {
                ...
                // Protocol fields: Kp, highPWM, lowPWM, minPWM, countsPerDeg,
                //                    wasOffset (2 bytes), ackerman
                // Note: The AgOpenGPS protocol does NOT send Ki, Kd.
                // We only update Kp and PWM bounds from AgIO.
                uint16_t wasOffset = (uint16_t)settings.wasOffsetLo
                                   | ((uint16_t)settings.wasOffsetHi << 8);

                controlUpdateSettings(settings.kp, settings.highPWM,
                                     settings.lowPWM, settings.minPWM);

                // Store settings in global state
                {
                    StateLock lock;
                    g_nav.settings_kp        = settings.kp / 10.0f;
                    g_nav.settings_ki        = 0.0f;   // not sent by AgIO
                    g_nav.settings_kd        = 0.0f;   // not sent by AgIO
                    g_nav.settings_min_pwm   = settings.minPWM;
                    g_nav.settings_max_pwm   = settings.highPWM;
                    g_nav.settings_counts    = settings.countsPerDeg;
                    g_nav.settings_was_offset = (int16_t)wasOffset;
